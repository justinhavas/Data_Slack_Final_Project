...
struct RUU_station {
  ...
  /* flag for allowing instruction with non-ready dependency to enter readyq */
  int allow_slack;
  /* Am I a parent of slack? */
  struct RS_link *parent_slack;
  // /* issue_lat */
  // int issue_lat;
  // /* op_lat */
  // int op_lat;
...
static void
eventq_queue_event(struct RS_link *node, struct RUU_station *rs, tick_t when)
{
  struct RS_link *prev, *ev, *new_ev; /*, *olink2, *node2; */
  // int i;

  if (rs->completed)
    panic("event completed");

  if (!rs->allow_slack && when <= sim_cycle*10)
  ...
  // {
  //   for (olink2=rs->odep_list[i]; olink2; olink2=olink2->next)
  //   {
  //     if (RSLINK_VALID(olink2))
  //     {
  //       struct RUU_station *olink_rs = RSLINK_RS(olink2);
  //       olink_rs->parent_slack = new_ev;
  //     }
  //   }
  // }
  // if (rs->allow_slack)
  ...
  //   node2 = node;
  //   while (node2 != NULL) {
  //     struct RUU_station *node_rs = RSLINK_RS(node2);
  //     if (node_rs->parent_slack != NULL && node_rs->parent_slack == node)
  //     {
  //       node_rs->parent_slack = new_ev;
  //       fprintf(stderr, "%s\n", "SET");
  //     }
  //     node2 = node2->next;
  //   }
...
eventq_next_event(void)
{
  ...
  if (event_queue && event_queue->x.when <= sim_cycle*10)
    {
  ...
  while ((rs = eventq_next_event()))
    {
      ...
      if ((!OPERANDS_READY(rs) && !rs->allow_slack) || rs->queued || !rs->issued || rs->completed)
...
ruu_issue(void)
{
  ...
  for (n_issued=0;
      ...
      if (RSLINK_VALID(node))
	{
	  ...
	  if ((!OPERANDS_READY(rs) && !rs->allow_slack) || !rs->queued
	  ...
	  if (rs->in_LSQ
	      ...
	      if (MD_OP_FUCLASS(rs->op) != NA)
		{
		  ...
		  if (fu)
			...
			panic("functional unit already in use");
              ...
              /* schedule functional unit release event */
			  if (rs->allow_slack && rs->parent_slack != NULL)
			  {
			  	/* HAHAHA make this equal to parent issuelat plus 1 */
          // struct RUU_station *par3 = RSLINK_RS(rs->parent_slack);
          // if (par3 != NULL)
          // {
          //   fu->master->busy = par3->issue_lat + 1;
          //    fprintf(stderr, "%s\n", "MADE IT"); 
          // }
          // else {
          //    fprintf(stderr, "%s\n", "NULL");
          //   fu->master->busy = fu->issuelat + 1;
          // }
          fu->master->busy = rs->parent_slack->issue_lat + fu->issuelat;
			  }
		      else { 
		      	fu->master->busy = fu->issuelat;
		      }

          node->issue_lat = fu->master->busy;
			      ...
			      == (F_MEM|F_LOAD)))
			{
			  ...
			  eventq_queue_event(node, rs, sim_cycle*10 + load_lat*10);
        ...
        tick_t my_op_lat;
			  if (rs->allow_slack && rs->parent_slack != NULL)
			  {
			  	/* HAHAHA make this equal to parent oplat plus my oplat */
          my_op_lat = rs->parent_slack->op_lat + fu->oplat*10;
			  	eventq_queue_event(node, rs, rs->parent_slack->op_lat + fu->oplat*10);
			  }
			  else {
          my_op_lat = sim_cycle*10 + fu->oplat*10;
				  eventq_queue_event(node, rs, sim_cycle*10 + fu->oplat*10);
			  }

        node->op_lat = my_op_lat;
	      ...
	      else /* does not require a functional unit! */
		{
		  ...
		  eventq_queue_event(node, rs, sim_cycle*10 + 1*10);
...
ruu_dispatch(void)
{
  ...
  int slack_parent = FALSE;     /* whether there is a slack parent */
  int parent_out1;  
  int parent_out2;    
...
#include "machine.def"
      ...
      if (op != MD_NOP_OP)
	{
	  ...
	  rs->seq = ++inst_seq;
	  rs->allow_slack = rs->queued = rs->issued = rs->completed = FALSE;
    rs->parent_slack = NULL;
    // rs->issue_lat = rs->op_lat = 0;
	  rs->ptrace_seq = pseq;
	  ...
	  if (MD_OP_FLAGS(op) & F_MEM)
        ...
        /* HAHAhA iterate through parent queue, if any outputs match inputs add */
      if (MD_OP_FUCLASS(rs->op) == IntALU)
      {
        ...
        if (RSLINK_VALID(node))
        {
          struct RUU_station *par = RSLINK_RS(node);
          if (par->onames[0] == in1 || par->onames[0] == in2 || par->onames[0] == in3 || par->onames[1] == in1 || par->onames[1] == in2 || par->onames[1] == in3)
          {
            if (MD_OP_FUCLASS(par->op) == IntALU)
            {
              rs->parent_slack = node;
            }
          }
        }
        node = node->next;
        ...
        {
          /* can not do slack for NULL parent */
